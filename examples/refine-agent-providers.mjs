...
const reviewResults = /** @type {Array<any>} */ (await parallel(
  PROVIDERS.map(name => () =>
    agent(
...
const synthesis = /** @type {{ summary: string, actionItems: Array<any> }} */ (await agent(
...
- verification: commands run or verification performed`,
    ...
    async update => {
      const updateReport = /** @type {{ summary: string, filesChanged: Array<string>, skipped: Array<any>, verification: Array<string> }} */ (update)
      phase('UpdateReview')
      log(`UpdateReview iteration ${iteration}: reviewing generated changes in ${updateReport.filesChanged.length} file(s)`)

      const updateReview = /** @type {{ approved: boolean, summary: string, findings: Array<any>, followUpActions: Array<string> }} */ (await agent(
...
- followUpActions: recommended non-code next steps, if any`,
      ...
      ) ?? { approved: false, summary: '', findings: [], followUpActions: [] })

      return { iteration, actionItems: actionItemsForIteration, update: updateReport, updateReview }
    }
  )
